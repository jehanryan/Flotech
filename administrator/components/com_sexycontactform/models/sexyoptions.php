...
class SexyContactFormModelSexyOptions extends JModelList {
	 ...
	 *
	 */
	public function getSexyForms() {
		$db		= $this->getDbo();
		$sql = "SELECT `id`, `name` FROM `#__sexy_forms` WHERE `published` <> '-2' order by `ordering`,`name` ";
		$db->setQuery($sql);
	 ...
	 *
	 */
	public function getSexyFields() {
		$db		= $this->getDbo();
		//$sql = "SELECT `id`, `name` FROM `#__sexy_fields` WHERE `published` <> '-2' ";
		$sql = "SELECT sfi.name as field_name, sfi.id as id, sf.name as form_name,sf.id as id_form, CONCAT(sfi.name,' - (FORM: ', sf.name, ')') as name FROM #__sexy_fields as sfi JOIN #__sexy_forms as sf ON sf.id = sfi.id_form WHERE sfi.published != -2 ORDER BY id_form,id";
	...
	protected function getListQuery()
	{
		...
		$query->from('#__sexy_options AS sp');
		
	
		// Join over the fields.
		$query->select('sfi.name AS field, sfi.id AS field_id');
		$query->join('LEFT', '#__sexy_fields AS sfi ON sfi.id=sp.id_field');
		
		// Join over the forms.
		$query->select('sf.name AS form, sf.id AS form_id');
		$query->join('LEFT', '#__sexy_forms AS sf ON sf.id=sfi.id_form');
		...
		if (is_numeric($formId)) {
			$query->join('INNER', '#__sexy_fields AS sfi1 ON sfi1.id=sp.id_field AND sfi1.id_form = '. (int)$formId);
