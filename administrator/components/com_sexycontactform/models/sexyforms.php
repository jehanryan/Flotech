...
class SexyContactFormModelSexyForms extends JModelList {
	...
	protected function getListQuery()
	{
		...
		$query->from('#__sexy_forms AS sp');
		
		// Join over the answers.
		$query->select('COUNT(sf.id) AS num_fields');
		$query->join('LEFT', '#__sexy_fields AS sf ON sf.id_form=sp.id AND sf.published!=-2');
