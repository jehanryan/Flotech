...
class SexyContactFormModelSexyFields extends JModelList {
	 ...
	 *
	 */
	public function getSexyTypes() {
		$db		= $this->getDbo();
		$sql = "SELECT `id`, `name` FROM `#__sexy_field_types`";
		$db->setQuery($sql);
		return $opts = $db->loadObjectList();
	}
	...
	protected function getListQuery()
	{
		...
		$query->from('#__sexy_fields AS sp');
		
		// get only published polls fields
		$query->join('INNER', '#__sexy_forms AS sp1 ON sp1.id=sp.id_form AND sp1.published <> -2');
		
		// Join over the forms.
		$query->select('sf.name AS form, sf.id AS form_id');
		$query->join('LEFT', '#__sexy_forms AS sf ON sf.id=sp.id_form');
		
		// Join over the types.
		$query->select('st.name AS type, st.id AS type_id');
		$query->join('LEFT', '#__sexy_field_types AS st ON st.id=sp.id_type');
